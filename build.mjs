...
function selftest() {
  ...
  ok('부정어가 극성을 뒤집는다', near(s('안 올랐다'), -2 / 2.2));
  ok('상하한 -1..+1 로 잘린다', s('급등 떡상 대박 신고가') === 1);
  ok('긍부정이 섞이면 상쇄', s('호재 악재') === 0);
  ...
  const daily = aggregate([
    { at: '2026-08-13T10:00:00-04:00', text: '호재' },      // 목 정규장
    { at: '2026-08-13T18:00:00-04:00', text: '악재' },      // 목 마감 후
    { at: '2026-08-15T11:00:00-04:00', text: '급등' },      // 토 → 월(17)로
