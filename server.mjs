...
// 한 번 도는 데 보통 0.6초라 겹치지는 않는다. 그래도 아래에 겹침 방지가 있다.
//
// 5초로 두고 9일 돌렸더니 토스가 서버 IP 를 통째로 막았다 — 종목당 시간당 720회다.
// 사람이 앱을 보는 속도가 아니다. 30초면 시간당 120회로 여섯 분의 일이고,
// 커뮤니티 글이 초 단위로 쏟아지지도 않아 화면에서 느껴지는 차이는 거의 없다.
const POLL_MS = Math.max(2_000, (Number(pollArg >= 0 ? argv[pollArg + 1] : 30) || 30) * 1000);
...
// 기동 직후 60분 창을 채울 때의 상한. 60페이지(660건)를 종목마다 몰아받으면
// 재시작 한 번이 크롤러처럼 보인다 — 실제로 그렇게 IP 가 막혔다. 디스크에 쌓아 둔
// 줄을 primeLive 가 먼저 채우므로, 남는 구멍만 메우면 된다.
const WARMUP_PAGES = 10;
